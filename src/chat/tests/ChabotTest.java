...
public class ChabotTest
{
	...
	{
		assertTrue("The content checker should definitly find the value", testedBot.contentChecker((String) testedBot.getContent()));
		...
		assertTrue("If the content is sent to the chatbot via process text it should be noted", testedBot.processText((String) testedBot.getContent()).contains("You said the special words"));
	}
	@SuppressWarnings("unchecked")
	@Test
	public void testGetSpookyList()
	{
		 assertNotNull("You really need to initialize the spookyList", testedBot.SpookyList());
	...
	{
		assertTrue("The responseList is not properly built by the constructor", ((ArrayList<String>) testedBot.ResponseList).size() > 15);
		assertTrue("The spookyList is not properly built by the constructor", ((ArrayList<String>) testedBot.SpookyList).size() > 8);
	}
